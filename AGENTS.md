...
│   ├── __init__.py          # ApiTestApp (combines mixins)
│   ├── ui_builder.py        # Layout, theme, widget wiring
│   ├── request_handling.py  # HTTP send/cancel/display
│   ├── preset_handling.py   # Preset load/save/batch queue
│   ├── settings_handling.py # Persist/restore UI state
│   └── dialogs.py           # Multi-select dialog
├── managers/                # Business logic (Qt-free)
│   ├── requests_manager.py  # QThread worker + manager
